...
public class WeightedQuickUnionUF {
    ...
    /** Initialize an empty union-find data structure with N isolated components 0 through N-1. 
@throws java.lang.IllegalArgumentException if N < 0
@param N the number of objects
    */
    public WeightedQuckUnionUF(int N) {
	count = N;
	parent = new int[N];
	size = new int[N];
	for (int i = 0; i < N; i++) {
...
@return true if the two sites p and q are in the same component, and 
false otherwise
@throw java.lang.IndexOutOfBoundsException unless both 0 <= p < N and 0 <= q < N */
...
@param p the integer representing one site
@param q the integer representing the other site
@throws java.lang.IndexOutOfBoundsException unless both 0 <= p < N and 0 <= q < 
    */
    public union(int p, int q) {
	int rootP = find(p);
	int rootQ = find(q);
	if(rootP == rootQ) reuturn;
    ...
    /** Reads in a sequence of pairs of integers (between 0 and N-1) from standard input, where each integer represents some object;
